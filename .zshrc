...
_openclaw_onboard() {
  _arguments -C \
    ...
    "--mode[Wizard mode: local|remote]" \
    "--auth-choice[Auth: setup-token|token|chutes|openai-codex|openai-api-key|openrouter-api-key|ai-gateway-api-key|moonshot-api-key|kimi-code-api-key|synthetic-api-key|venice-api-key|gemini-api-key|zai-api-key|xiaomi-api-key|apiKey|minimax-api|minimax-api-lightning|opencode-zen|skip]" \
...
_openclaw_message_send() {
  _arguments -C \
    ...
    "--channel[Channel: telegram|whatsapp|discord|googlechat|slack|signal|imessage|nostr|msteams|mattermost|nextcloud-talk|matrix|bluebubbles|line|zalo|zalouser|tlon]" \
...
_openclaw_agent() {
  _arguments -C \
    ...
    "--channel[Delivery channel: last|telegram|whatsapp|discord|googlechat|slack|signal|imessage|nostr|msteams|mattermost|nextcloud-talk|matrix|bluebubbles|line|zalo|zalouser|tlon (default: whatsapp)]" \
...
_openclaw_completion() {
  _arguments -C \
    "(--shell -s)"{--shell,-s}"[Shell to generate completion for]" \
...
_openclaw_cron_add() {
  _arguments -C \
    ...
    "--deliver[Deliver agent output (required when using last-route delivery without --to)]" \
    ...
    "--post-prefix[Prefix for main-session post]" \
    "--post-mode[What to post back to main for isolated jobs (summary|full)]" \
    "--post-max-chars[Max chars when --post-mode=full (default 8000)]" \
    "--json[Output JSON]" \
    "--url[Gateway WebSocket URL (defaults to gateway.remote.url when configured)]" \
    "--token[Gateway token (if required)]" \
    "--timeout[Timeout in ms]" \
...
_openclaw_cron_edit() {
  _arguments -C \
    ...
    "--post-prefix[Prefix for summary system event]" \
    "--url[Gateway WebSocket URL (defaults to gateway.remote.url when configured)]" \
    "--token[Gateway token (if required)]" \
    "--timeout[Timeout in ms]" \
    "--expect-final[Wait for final response (agent)]"
...
_openclaw_channels_capabilities() {
  _arguments -C \
    "--channel[Channel (all|telegram|whatsapp|discord|googlechat|slack|signal|imessage|nostr|msteams|mattermost|nextcloud-talk|matrix|bluebubbles|line|zalo|zalouser|tlon)]" \
